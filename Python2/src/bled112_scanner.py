...
def main():
        ...
        def format_option(self, option):
            ...
            if option.help:
                ...
                for para in help_text.split("\n"):
                    help_lines.extend(
                        optparse.textwrap.wrap(
                            para, self.help_width))
                result.append("%*s%s\n" % (
    ...
    p = MyParser(description='Bluetooth Smart Scanner script for Bluegiga BLED112 v2013-03-30', epilog="""Examples:
...
"""
    ...
    group.add_option(
        ...
        '-p',
        type="string",
        help="Serial port device name (default /dev/ttyACM0)",
        metavar="PORT")
    group.add_option(
        '--baud',
        '-b',
    ...
    group.add_option(
        ...
        '-i',
        type="int",
        help="Scan interval width in units of 0.625ms (default 200)",
        metavar="INTERVAL")
    group.add_option(
        '--window',
        '-w',
        type="int",
        help="Scan window width in units of 0.625ms (default 200)",
        metavar="WINDOW")
    group.add_option(
        '--active',
        '-a',
        action="store_true",
        help="Perform active scan (default passive)\nNOTE: active scans result "
    ...
    group.add_option(
        ...
        type="string",
        action="append",
        help="MAC address(es) to match",
        metavar="ADDRESS")
    group.add_option(
        '--rssi',
        '-r',
        type="int",
        help="RSSI minimum filter (-110 to -20), omit to disable",
        metavar="RSSI")
    p.add_option_group(group)
    ...
    group.add_option(
        ...
        '-q',
        action="store_true",
        help="Quiet mode (suppress initial scan parameter display)")
    group.add_option('--friendly', '-f', action="store_true",
                     help="Friendly mode (output in human-readable format)")
    group.add_option(
        '--display',
        '-d',
        type="string",
        help="Display fields and order (default '%default')\n"
...
def bgapi_parse(b):
    ...
    if bgapi_rx_expected_length > 0 and len(
            bgapi_rx_buffer) == bgapi_rx_expected_length:
            ...
            if packet_class == 0x06:  # gap
                if packet_command == 0x00:  # scan_response
                    ...
                    if display and len(filter_uuid) > 0:
                        if not [i for i in filter_uuid if i in ad_services]:
                            display = 0
                    ...
                    if display:
                        ...
                        for c in options.display:
                            if c == 't':
                                ...
                                disp_list.append("%s" % ''.join(
                                    ['%02X' % b for b in sender[::-1]]))
                            elif c == 'a':
