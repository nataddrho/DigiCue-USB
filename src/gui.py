...
class ResizablePlotPoint(Resizable):
	def __init__(self,canvas,x0,y0,mag,**kwargs):
		...
		self.mag = mag
		self.size = 3
		self.object = canvas.create_oval(x0-self.size,y0-self.size,x0+self.size,y0+self.size,**kwargs)	
	...
	def redraw(self,**kwargs):
		...
		self.canvas.coords(self.object,a-self.size,b-self.size,a+self.size,b+self.size,**kwargs)
...
class ScoreBar:
	...
	def __init__(self,canvas,left,top,right,bottom,scale):	
		...
		self.scorebar = ResizableRectangle(canvas,0,0,0,0,fill="red",outline="")
		self.avgbar = ResizableRectangle(canvas,0,0,0,0,fill="dark gray",outline="")		
	...
	def update(self,enabled=True):
		...
		if self.points==1:
			...
			self.average = self.average*(self.points-1)/float(self.points) + self.score/float(self.points)
		...
		self.avgbar.redraw(self.left,self.bottom,avg,self.bottom+(self.bottom-self.top)*0.25)
		self.thresholdline.redraw(thresh,self.top,thresh,self.bottom)
...
class Plot:
	...
	def __init__(self,canvas,left,top,size):
		...
		ResizableRectangle(self.canvas,self.left,self.top,self.left+self.size,self.top+self.size)
		ResizableLine(self.canvas,self.left,self.top+self.size/2,self.left+self.size,self.top+self.size/2)
		ResizableLine(self.canvas,self.left+self.size/2,self.top,self.left+self.size/2,self.top+self.size)
		...
		self.y0 = y0
		r = 0.5
		self.threshold_circle = ResizableOval(self.canvas,x0-r,y0-r,x0+r,y0+r)		
	...
	def plot(self,x,y,**kwargs):
		...
		self.points.append(ResizablePlotPoint(self.canvas,x,y,mag,**kwargs))
		
	def update(self,threshold):
		self.threshold = threshold		
		r = threshold*self.size/2
		self.threshold_circle.redraw(self.x0-r,self.y0-r,self.x0+r,self.y0+r)	
...
class ScoreBars:
	...
	def __init__(self,frame, dcb):
		...
		self.canvas = ResizableCanvas(self.frame, width=810, height=400, highlightthickness=0)
		...
		self.bars = []	
		self.scores = []
		self.labels = ('Finish','Finesse','Straightness','Tip Steer','Follow Through','Jab','Backstroke Pause','Shot Interval')
		self.scale = (3.0,10.0,10.0,10.0,10.0,10.0,1.0,60.0)
		self.score_format = ("%0.2fs","%0.1f","%0.1f","%0.1f","%0.1f","%0.1f","%0.2fs","%0.1fs")
		self.outof = ('/3s','/10','/10','/10','/10','/10','/1s','/60s')
		for i in xrange(0,8):
			top = i*400/8.+10
			bottom = (i+1)*400/8.-10			
			self.bars.append(ScoreBar(self.canvas,left,top,right,bottom,self.scale[i]))			
			ResizableRectangle(self.canvas,10,top,left-10,bottom)
			ResizableRectangle(self.canvas,left-10,top,left-100,bottom)
			ResizableText(self.canvas,100,top+(bottom-top)*0.5,text=self.labels[i],font=("Purisa",14))			
			ResizableText(self.canvas,575,top+(bottom-top)*0.5,text=self.outof[i],font=("Purisa",14))
			self.scores.append(ResizableText(self.canvas,left-55,top+(bottom-top)*0.5,text="0",font=("Purisa",18)))
			
		
		#Plot area
		self.plot = Plot(self.canvas,600,100,200)
		ResizableText(self.canvas,700,75,text="Straightness",font=("Purisa",14))
...
class GUI:
	...
	def __init__(self,master,dcb):
		...
		#All variables from DigiCue Blue are exposed through class variables in dcb
		...
		self.macaddrs_combo = Tk.OptionMenu(frame,self.macaddrs,"<Auto Detect>")		
		...
		for label,modes in dcb.config_options:				
			...
			lbl.pack(side=Tk.LEFT)		
			v = Tk.StringVar()
			b = Tk.Radiobutton(frame,text="Off",variable=v,value=-1,indicatoron=0,width=10,command=self.check_setting_config)
			b.pack(side=Tk.LEFT)
			for text,mode in modes:	
				if text<>"Off":
					b = Tk.Radiobutton(frame,text=text,variable=v,value=mode,indicatoron=0,width=10, command=self.check_setting_config)
					b.pack(side=Tk.LEFT)
	...
	def refresh_setting_config(self):					
		self.options_configig["Shot Interval"].set(self.dcb.threshset_shotpause if self.dcb.setting_shotpause else -1)
		self.options_configig["Backstroke Pause"].set(self.dcb.threshset_bspause if self.dcb.setting_bspause else -1)
		self.options_configig["Jab"].set(self.dcb.threshset_jab if self.dcb.setting_jab else -1)
		self.options_configig["Follow Through"].set(self.dcb.threshset_followthru if self.dcb.setting_followthru else -1)
		self.options_configig["Tip Steer"].set(self.dcb.threshset_steering if self.dcb.setting_steering else -1)
		self.options_configig["Straightness"].set(self.dcb.threshset_straightness if self.dcb.setting_straightness else -1)		
		self.options_configig["Finesse"].set(self.dcb.threshset_power if self.dcb.setting_power else -1)
		self.options_configig["Finish"].set(self.dcb.threshset_freeze if self.dcb.setting_freeze else -1)
		self.options_configig["Vibrate On Pass"].set(self.dcb.setting_vop if self.dcb.setting_vop else -1)
		self.options_configig["Disable All Vibrations"].set(self.dcb.setting_dvibe if self.dcb.setting_dvibe else -1)
	...
	def check_setting_config_test(self):	
		...
		a=0
		val = lambda x: -2 if len(x)==0 else x
	...
	def refresh_macaddrs(self):		
		...
		for choice in self.macaddrs_list:
			...
			self.macaddrs_combo['menu'].add_command(label=choice,command=command)			
		
		
	def timer(self):
		if not self.dcb.macaddr in self.macaddrs_list:
			if self.dcb.macaddr<>None:				
				self.macaddrs_list.append(self.dcb.macaddr) #Only add DigiCue Blue devices / correct manuf. ID				
