...
import bglib, serial, time, datetime, optparse, signal, sys, struct
...
class Bluegiga():
	...
	def initialize(self):
		...
		self.uuid_crp_service = [0x0b, 0xd5, 0x16, 0x66, 0xe7, 0xcb, 0x46, 0x9b, 0x8e, 0x4d, 0x27, 0x42, 0xf1, 0xba, 0x77, 0xcc]
		self.uuid_crp_characteristic = [0xe7, 0xad, 0xd7, 0x80, 0xb0, 0x42, 0x48, 0x76, 0xaa, 0xe1, 0x11, 0x28, 0x55, 0x35, 0x3c, 0xc1]
	...
	def my_ble_evt_gap_scan_response(self,sender, args):
		...
		if self.uuid_crp_service in ad_services:			
			...
			if not args['sender'] in self.peripheral_list:
				...
				self.ble.send_command(self.ser, self.ble.ble_cmd_gap_connect_direct(args['sender'], args['address_type'], 0x06, 0x10, 0x100, 0))
	...
	def my_ble_evt_connection_status(self,sender, args):
		...
		if (args['flags'] & 0x05) == 0x05:
			...
			self.remoteAddressString = ':'.join(['%02X' % b for b in args['address'][::-1]])
			self.dprint("Connected to %s" % self.remoteAddressString)
			self.connection_handle = args['connection']
			self.ble.send_command(self.ser, self.ble.ble_cmd_attclient_read_by_group_type(args['connection'], 0x0001, 0xFFFF, list(reversed(self.uuid_service))))
			self.ble.check_activity(self.ser, 1)
	...
	def my_ble_evt_attclient_group_found(self,sender, args):
		...
		if args['uuid'] == list(reversed(self.uuid_crp_service)):
			self.dprint("Found attribute group for CRP service: start=%d, end=%d" % (args['start'], args['end']))
	...
	def my_ble_evt_attclient_find_information_found(self,sender, args):
		...
		if args['uuid'] == list(reversed(self.uuid_crp_characteristic)):
			self.dprint("Found CRP data attribute: handle=%d" % args['chrhandle'])
		...
		# check for subsequent client characteristic configuration
			...
			self.dprint("Found CRP client characteristic config attribute w/UUID=0x2902: handle=%d" % args['chrhandle'])
	...
	def my_ble_evt_attclient_procedure_completed(self,sender, args):		
		...
		# check if we just finished searching for attributes within the CRP service
			...
			if self.att_handle_data_ccc > 0:
				...
				self.ble.send_command(self.ser, self.ble.ble_cmd_attclient_attribute_write(self.connection_handle, self.att_handle_data_ccc, [0x02, 0x00]))
				self.ble.check_activity(self.ser, 1)
	...
	def main_loop(self):
		...
		while (self.disconnected==0):	
			...
			if self.crp_link_ready and not self.pending_write:
				...
				if not self.init_sent and self.dcb.pendACONF0<>None:
					#send configuration data (16 bytes)
					self.init_sent = True				
					configuration_data = [self.dcb.pendACONF0,self.dcb.pendACONF1,self.dcb.pendACONF2,self.dcb.pendACONF3]
					chksum = 0
					...
					self.ble.send_command(self.ser, self.ble.ble_cmd_attclient_attribute_write(self.connection_handle, self.att_handle_data, configuration_data))
