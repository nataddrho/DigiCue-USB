...
import math, struct, datetime, math, os
...
class DigicueBlue():
	...
	def set_config(self,configuration):
		...
		self.pendACONF0 = (setting[7]<<7) | (setting[6]<<6) | (setting[5]<<5) | (setting[4]<<4) | (setting[3]<<3) | (setting[2]<<2) | (setting[1]<<1) | setting[0]  
		self.pendACONF1 = ((thresh[3] & 0x03)<<6) | ((thresh[2] & 0x03)<<4) | ((thresh[1] & 0x03)<<2) | (thresh[0] & 0x03)
		self.pendACONF2 = ((thresh[7] & 0x03)<<6) | ((thresh[6] & 0x03)<<4) | ((thresh[5] & 0x03)<<2) | (thresh[4] & 0x03)
	...
	def receive(self,mac,data):	
		...
		if data[0:3]<>[0x02,0x01,0x06] or data[10]<0x04 or data[11:14]<>[0xFF,0x03,0xDE]:
			return
		...
		if data_type==0: #Version
			...
			self.impactx = -impactmag*math.cos(math.pi/180*impactang-math.pi/2)
			self.impacty = -impactmag*math.sin(math.pi/180*impactang-math.pi/2)
			...
			for k in xrange(0,10):
				i = k*85/9.0
				if (impactang>(90-i) and impactang<(90+i)) or (impactang>(270-i) and impactang<(270+i)):
					break
				else:
					j+=1
			if impactmag<=10:
